...
function FieldBase({
  ...
  const renderLabel = (label && showLabel) || required || tooltip || repeatable;
      ...
      >
        ...
        {renderLabel && (
          <>
            {showLegend ? (
              <fieldset>
                <legend {...accessibleProps} className="lfr-ddm-legend">
        ...
        {localizedValueArray.length > 0 &&
          localizedValueArray.map((language) => (
            <input
              ...
              type="hidden"
              value={
                language.value ? convertInputValue(type, language.value) : ""
              }
            />
          ))}
