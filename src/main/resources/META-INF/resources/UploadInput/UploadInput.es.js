        ...
        <ClayInput.GroupItem append shrink>
          <label
            className={
              ...
              (transformedFileEntryTitle ? " clear-button-upload-on" : "") +
              (readOnly ? " disabled" : "")
            }
...
const Main = ({
  ...
  const getErrorMessages = (errorMessage, isSignedIn) => {
    ...
    if (!allowGuestUsers && !isSignedIn) {
      errorMessages.push(
        Liferay.Language.get("you-need-to-be-signed-in-to-edit-this-field")
      );
    ...
    axios
      ...
      .then((response) => {
        ...
        if (error) {
          ...
          handleGuestUploadFileChanged("", event, JSON.stringify(file));
        }
